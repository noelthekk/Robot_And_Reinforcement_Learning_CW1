...
if __name__ == "__main__":
    ...
    FIXED_H = 50 # Increased to help the controller "see" the sine wave peaks
    num_evals = 20    
    max_steps = 500    
    DISTURBANCE = 0
    ...
    for param_name, test_values in variations.items():
        for val in test_values:
            ...
            # Setup play_env
            play_env = CartPoleEnv(render_mode="human", 
                                  x_threshold=X_LIMIT, 
            ...
            play_env = RecordVideo(play_env, 
                                   ...
                                   episode_trigger=lambda ep: ep < 1) # Just record 1 video to save time
    ...
    fig, axes = plt.subplots(1, len(variations), figsize=(22, 5), sharey=True)
    for i, (param, data) in enumerate(sensitivity_data.items()):
        axes[i].plot(data['values'], data['rates'], marker='o', color='crimson', linestyle='--')
        axes[i].set_title(f'Failure Rate vs {param}')
        axes[i].set_xlabel('Weight Value')
        axes[i].set_ylim([-0.05, 1.05])
        if i == 0: axes[i].set_ylabel('Termination Rate')
        axes[i].grid(True, alpha=0.3)
    
    plt.tight_layout()
    pdf_filename = 'weight_sensitivity_fixed.pdf'
...
if __name__ == "__main__":
    ...
    max_steps = 500 
    DISTURBANCE = 0
    base_params = {'wx': 0.2, 'wv': 0.1, 'wth': 30.0, 'wthd': 1.0, 'wu': 0.002}
    
    # Parameters to vary one-at-a-time
    variations = {
        'wx': [0.05, 0.2, 1.0],     # Cart position constraint
        'wv': [0.01, 0.1, 1.0],     # Cart velocity damping
        'wth': [10.0, 30.0, 50.0],   # Tracking Stiffness
        'wthd': [0.1, 1.0, 5.0],    # Damping of pole velocity
        'wu': [0.0001, 0.002, 0.1]    # Control effort penalty
    }

    # Dictionary to store results for the final summary plots
    sensitivity_data = {param: {'values': [], 'costs': []} for param in variations}
    ...
    for param_name, test_values in variations.items():
        for val in test_values:
            ...
            # Setup Env (Recording only the very first variation to save time)
            ...
            for ep in range(num_evals):
                ...
                for t in range(max_steps):
                    action = mpc.control(state)
                    state, _, terminated, truncated, _ = play_env.step(action)
                    # Use the same stage_cost from your class for consistency
                    ep_cost += mpc.stage_cost(state, [action], mpc.t0)
                    if terminated: break
                
                total_costs_in_variation.append(ep_cost)
            
            # Log the mean cost for this parameter value
            mean_val_cost = np.mean(total_costs_in_variation)
            sensitivity_data[param_name]['values'].append(val)
            sensitivity_data[param_name]['costs'].append(mean_val_cost)
            play_env.close()

    # --- 2. Generate Sensitivity Summary Plots ---
    fig, axes = plt.subplots(1, len(variations), figsize=(20, 5), sharey=True)
    for i, (param, data) in enumerate(sensitivity_data.items()):
        axes[i].plot(data['values'], data['costs'], marker='o', color='teal')
        axes[i].set_title(f'Cost vs {param}')
        axes[i].set_xlabel('Weight Value')
        if i == 0: axes[i].set_ylabel('Mean Episode Cost')
        axes[i].grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig('weight_sensitivity_summary.png')
    plt.show()
...
if __name__ == "__main__":
    ...
    FIXED_H = 50 
    num_evals = 5 
    max_steps = 500 # Requirement for Problem 5 [cite: 234]
    DISTURBANCE = 0

    base_params = {
        'wx': 0.2, 'wv': 0.1, 'wth': 30.0, 'wthd': 1.0, 'wu': 0.002
    }

    # Parameters to vary one-at-a-time
    variations = {
        'wth': [10.0, 50.0],   # Tracking Stiffness
        'wthd': [0.1, 5.0],    # Damping of pole velocity
        'wx': [0.05, 1.0],     # Cart position constraint
        'wv': [0.01, 1.0],     # Cart velocity damping
        'wu': [0.0001, 0.1]    # Control effort penalty
    }
    ...
    for param_name, test_values in variations.items():
        for val in test_values:
            ...
            # Initialize MPC with keyword expansion (**current_params)
            mpc = CartpoleMPC(H=FIXED_H, **current_params)
            ...
            print(f"Result for {param_name}={val}: Success Rate {(num_evals-terminated_count)/num_evals:.2%}")
            # if len(mpc.history) > 0:
            #     plot_p5_analysis(mpc)
            play_env.close()
...
if __name__ == "__main__":
    ...
    #                                  make rgb_array to human for the output window
    play_env = CartPoleEnv(render_mode="rgb_array", x_threshold=X_LIMIT, theta_threshold_radians=THETA_LIMIT, continuous_action=True, disturbance=DISTURBANCE)
    play_env = TimeLimit(play_env, max_episode_steps=MAX_EPISODE_STEPS)
    play_env = RecordVideo(play_env, video_folder="videos", name_prefix="(H_MPC={}) mpc_swing", episode_trigger=lambda ep: True)
    ...
    if USE_MPC:
        ...
        while True:
            u_plan, x_plan, ks, Ks = mpc.solve_ilqr(state, np.zeros((H_ILQR, 1)), mpc.t0)
