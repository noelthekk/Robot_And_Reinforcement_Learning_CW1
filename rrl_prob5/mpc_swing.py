...
if __name__ == "__main__":
    ...
    variations = {
        # 'wx': np.linspace(0.05, 0.5, 10).tolist(),
        # 'wv': np.linspace(0.005, 0.05, 10).tolist(), 
        # 'wth': np.linspace(10.0, 200.0, 20).tolist(),
        'wthd': np.linspace(1, 10, 10).tolist(), 
        # 'wu': [0.0005, 0.001, 0.005] 
    }

    sensitivity_data = {param: {'values': [], 'mean_angle_error': []} for param in variations}
    ...
    for param_name, test_values in variations.items():
        for val in test_values:
            ...
            # Setup play_env
            play_env = CartPoleEnv(render_mode=None, 
                                  x_threshold=X_LIMIT, 
            ...
            for ep in range(num_evals):
                ...
                for t in range(max_steps):
                    action = mpc.control(state,t)
                    state, _, terminated, truncated, _ = play_env.step(action)
                    theta = state[2]
                    theta_ref, _ = mpc.ref_theta(mpc.t0)
                    angle_errors.append(np.abs(np.rad2deg(theta - theta_ref)))
                    if terminated:
                        terminated_count += 1
                        break
                    if truncated:
            ...
            # print(f"Termination Rate for {param_name}={val}: {rate:.2f}")
            print(f"Mean Angle Error for {param_name}={val}: {np.mean(angle_errors):.2f}°")
            sensitivity_data[param_name]['values'].append(val)
            sensitivity_data[param_name]['mean_angle_error'].append(np.mean(angle_errors))
            # play_env.close()
    ...
    for i, (param, data) in enumerate(sensitivity_data.items()):
        axes_flat[i].plot(data['values'], data['mean_angle_error'], marker='o', color='crimson', linestyle='--')
        axes_flat[i].set_title(f'Mean Angle Error vs {param}')
        axes_flat[i].set_xlabel('{param} Value')
        # axes_flat[i].set_ylim([0, 100])  # Adjusted y-axis to be more meaningful for angle errors
        
        # Only label the y-axis for the first plot in the row
        if i == 0: 
            axes_flat[i].set_ylabel('Mean Angle Error (degrees)')
    ...
    pdf_filename = f"{param}_tracking_error.pdf"
