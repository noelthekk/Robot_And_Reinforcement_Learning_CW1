...
def compute_policy(V):
    ...
    policy = np.argmin([quadratic_cost(state_tensor) + GAMMA * V[state_to_indices(dynamics(state_tensor, action), [x_vals, x_dot_vals, theta_vals, theta_dot_vals])] for action in ACTION_VALS], axis=0)
